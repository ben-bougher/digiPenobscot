...
class mudLine:
    ...
    def __init__(self, c, g, m,vp, 
                 ...
                 vp_sig=0, dvp_sig=500.0, n=100000):
    ...
    def get_vp(self):
        return self.vp + np.random.randn()*self.vp_sig
    ...
    def distribution_(self, n):
        ...
        H, xedges, yedges = np.histogram2d(G, I, bins=200,
                                           range=[[-1,1],[-1,1]])
...
class AvoGMM(BaseEstimator):
    ...
    def __init__(self, avoDist, n_gaussians=1, n_iter=100, n_init=1,
                 tol=1e-3,min_covar=1e-3, verbose=False):
    ...
    def _fit(self, X):
        ...
        for init in range(self.n_init):
            ...
            for i in range(self.n_iter):
                ...
                self.weights_ = _alpha_mstep(pk_xt)

                # Update the means
                self.means_ = _mu_mstep(X, pk_xt)

                # Update the covariances
                self.covars_ = _covar_mstep(X, pk_xt, self.means_)
...
def _mu_mstep(X, p_k):
    ...
    return np.dot(X.T, p_k).T / np.sum(p_k,0)
...
def _covar_mstep(X, p_k, mu_new):
    ...
    dot_diff = np.zeros(p_k.shape)
    # Calculate the numerator
    normalizer = np.sum(p_k,0)

    for i, mean in enumerate(mu_new):

        diff = X - mean
        dot_diff[:,i] = np.sum(diff*diff, 1)

    numerator = np.dot(p_k.T, dot_diff).T
...
def _step_log_likelihood(alpha_new, p_k, pX_new):
    ...
    alpha_term = np.sum(np.dot(np.log(alpha_new), p_k.T).T,0)
    theta_term = np.sum(np.log(pX_new)*p_k)
